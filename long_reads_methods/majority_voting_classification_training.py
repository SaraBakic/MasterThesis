...
def evaluate(model, eval_data, eval_labels, eval_lens, eval_splits, batch_size, device, tgt_path, epoch):
    predictions = []
    with torch.no_grad():
        for i in range(0, len(eval_data), batch_size):
            eval_batch = torch.cat(eval_data[i:i+batch_size]).to(device)
            lens = np.concatenate(eval_lens[i:i+batch_size])
            splits = eval_splits[i:i+batch_size]
            logits = model.forward(eval_batch, lens)
            logits = torch.split(logits, splits, 0)
            logits = torch.cat([torch.mean(i, 0, keepdim=True) for i in logits], 0)
...
def main(args, devices):
    ...
    train_data, train_splits = initialize_token_embeddings(vocab, train_data, args.seq_len)
    print(train_lens[:20])

    eval_df = pd.read_csv(args.eval_chunks)
    eval_data, eval_lens = tokenize_chunks(list(eval_df['Sequence']), args.k)
    eval_data, eval_splits = initialize_token_embeddings(vocab, eval_data, args.seq_len)
    ...
    for epoch in range(args.epochs):
        ...
        for i in tqdm(range(0, len(train_data), args.batch_size)):
            ...
            splits = train_splits[i:i+args.batch_size]
        ...
        evaluate(classification_model, eval_data, eval_labels, eval_lens, eval_splits, args.batch_size, devices[0], args.model_tgt, epoch)

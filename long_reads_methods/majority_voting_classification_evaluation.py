...
def initialize_token_embeddings(vocab, tokenized_sequences, seq_len):
    ...
    for seq in tokenized_sequences:
        ...
        if t.size()[1] > seq_len:
            ...
            data.append(torch.cat(splitted_t, dim=0))
...
def evaluate(model, eval_data, eval_labels, eval_lens, device, tgt_path, epoch=0):
    predictions = []
    with torch.no_grad():
        for i, sample in enumerate(eval_data):
            eval_batch = sample.to(device)
            lens = [512]*(eval_lens[i]//(512*9)) + [int(math.ceil(eval_lens[i] % (512*9) / 9))]
            if lens[-1] == 0:
                lens = lens[:-1]
            print(eval_batch.size(), lens, eval_lens[i])
            logits = model.forward(eval_batch, lens)
            print(logits)
            predicted = torch.argmax(logits, dim=1).to('cpu')
            predicted = torch.nn.functional.one_hot(predicted, num_classes=7)
            predicted = torch.sum(predicted, 0, keepdim=True)
...
def main(args, devices):
    ...
    evaluate(embedding_model, eval_data, eval_labels, full_lens, devices[0], f'./{args.type}_reads_')

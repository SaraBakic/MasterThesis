...
import numpy as np
import math
from averaging_classification_network import ClassificationNetwork
...
def tokenize_chunks(chunks, k):
    ...
    for i, chunk in enumerate(chunks):
        ...
        current_len = [512]*(len(tokenized_chunk)//512) + [int(math.ceil(len(tokenized_chunk) % 512))]
        if current_len[-1] < 0.65*512:
            current_len = current_len[:-1]
        lens.append(current_len)
    return tokenized_chunks, lens
...
def evaluate(model, eval_data, eval_labels, eval_lens, eval_slices, batch_size, device, tgt_path, epoch):
    predictions = []
    with torch.no_grad():
        for i in range(0, len(eval_data), batch_size):
            eval_batch = torch.cat(eval_data[i:i+batch_size]).to(device)
            lens = np.concatenate(eval_lens[i:i+batch_size])
            slices = eval_slices[i:i+batch_size]
            logits = model.forward(eval_batch, lens, slices)
...
def main(args, devices):
    ...
    train_data, train_lens = tokenize_chunks(list(train_df['Sequence']), args.k)
    train_data, train_slices = initialize_token_embeddings(vocab, train_data, args.seq_len)
    print(train_lens[:20])

    eval_df = pd.read_csv(args.eval_chunks)
    eval_data, eval_lens = tokenize_chunks(list(eval_df['Sequence']), args.k)
    eval_data, eval_slices = initialize_token_embeddings(vocab, eval_data, args.seq_len)
    ...
    for epoch in range(args.epochs):
        ...
        for i in tqdm(range(0, len(train_data), args.batch_size)):
            ...
            slices = train_slices[i:i+args.batch_size]
            batch_labels = train_labels[i:i+args.batch_size].to(devices[0])
            logits = classification_model.forward(batch, lens, slices)
        ...
        evaluate(classification_model, eval_data, eval_labels, eval_lens, eval_slices, args.batch_size, devices[0], args.model_tgt, epoch)
